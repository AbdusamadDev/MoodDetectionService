...
class MainStream:
    ...
    async def process_frames(self):
        ...
        while True:
            ...
            tasks = [
                self.face_recognition.process_face_and_mood(face) for face in faces
            ]
    ...
    async def reload_face_encodings_periodically(self):
        while True:
            ...
            ) = self.trainer.load_face_encodings(self.trainer.root_dir)
            print(
    ...
    async def reconnect_cameras_periodically(self, interval=20):
        ...
        while True:
            ...
            for url in database.get_camera_urls():
                print("Reconnecting to camera url: ", url)
...
async def main():
    ...
    reconnect_to_cameras = asyncio.create_task(
        await stream.reconnect_cameras_periodically()
    )
    await asyncio.gather(
        ws_server.wait_closed(),
        camera_streams_task,
        reload_encodings_task,
        reconnect_to_cameras
    )
