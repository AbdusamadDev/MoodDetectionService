...
class Database:
    def __init__(self):
        ...
        self.user = os.environ.get(
            "DB_USER"
        )  # Changed from "USER" to "DB_USER" to avoid conflict with system variables
    ...
    def get_details(self, employee_id):
        ...
        rows = self._execute_query(
            query, (employee_id,)
        )  # Changed parameter name to 'employee_id' for clarity
        labels = [
            "id",
            "first_name",
            "last_name",
    ...
    def get_by_similar(self, partial_url):
        ...
        rows = self._execute_query(
            query, (f"%{partial_url}%",)
        )  # Simplified the like_pattern logic
